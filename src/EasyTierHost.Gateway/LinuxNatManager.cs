...
public sealed partial class LinuxNatManager(ICommandRunner runner) : IGatewayPlatform
{
    ...
    public async Task<GatewayPlatformSnapshot> CaptureAsync(RouteSnapshot physical, GatewayAdapter overlay, CancellationToken ct)
    {
        ...
        await runner.CheckedAsync("nft", ["-j", "list", "tables"], ct);
        ...
        return new("eth_" + token, token, "linux", physical, overlay, forwarding == "1", []);
    }
    ...
    private async Task<bool> OwnedAsync(GatewayPlatformSnapshot s, CancellationToken ct)
    {
        ...
        return json.RootElement.GetProperty("nftables").EnumerateArray().Any(e => e.TryGetProperty("table", out var t) && t.TryGetProperty("comment", out var comment) && comment.GetString() == "EasyTierHost:" + s.OwnerToken);
    }
    ...
    {
        if ((await runner.CheckedAsync("sysctl", ["-n", "net.ipv4.ip_forward"], ct)).Trim() != "1" || !await TableExistsAsync(s, ct) || !await OwnedAsync(s, ct)) return false;
        using var json = JsonDocument.Parse(await runner.CheckedAsync("nft", ["-j", "list", "table", "ip", Name(s.ResourceName)], ct));
    ...
    public static bool HasExpectedRule(JsonElement json, GatewayPlatformSnapshot s)
    {
        ...
        {
            if (!item.TryGetProperty("rule", out var rule) || rule.GetProperty("chain").GetString() != "postrouting" || !rule.TryGetProperty("comment", out var comment) || comment.GetString() != "EasyTierHost:" + s.OwnerToken) continue;
