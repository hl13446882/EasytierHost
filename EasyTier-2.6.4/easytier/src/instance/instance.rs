...
impl Instance {
    ...
    fn check_dhcp_ip_conflict(&self) {
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                let range = global_ctx_c.config.get_dhcp_range();
                let dhcp_inet = range
                    .as_ref()
                    .map(|r| &r.network)
                    .unwrap_or_else(|| used_ipv4.iter().next().unwrap_or(&default_ipv4_addr));
                // if old ip is already in this subnet and not conflicted, use it
                if let Some(ip) = current_dhcp_ip
                    && ip.network() == dhcp_inet.network()
                    && range.as_ref().is_none_or(|r| r.contains(ip.address()))
                    && if range.is_some() {
                        !used_ipv4.iter().any(|used| used.address() == ip.address())
                    } else {
                ...
                let candidate_ipv4_addr = if let Some(range) = &range {
                    range.candidate(&used_ipv4)
                } else {
    ...
    pub async fn run(&mut self) -> Result<(), Error> {
        if let Some(range) = self.global_ctx.config.get_dhcp_range() {
            range.validate()?;
            if !self.global_ctx.config.get_dhcp() {
                return Err(anyhow::anyhow!("dhcp_range requires DHCP").into());
            }
        }
