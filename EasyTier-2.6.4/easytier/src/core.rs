...
struct NetworkOptions {
    ...
    dhcp: Option<bool>,
    #[arg(long, env = "ET_DHCP_START", requires_all = ["dhcp_end", "dhcp_network"])]
    dhcp_start: Option<std::net::Ipv4Addr>,
    #[arg(long, env = "ET_DHCP_END", requires_all = ["dhcp_start", "dhcp_network"])]
    dhcp_end: Option<std::net::Ipv4Addr>,
    #[arg(long, env = "ET_DHCP_NETWORK", requires_all = ["dhcp_start", "dhcp_end"])]
    dhcp_network: Option<cidr::Ipv4Inet>,
...
impl NetworkOptions {
    ...
    fn merge_into(&self, cfg: &TomlConfigLoader) -> anyhow::Result<()> {
        ...
        if let (Some(start), Some(end), Some(network)) =
            (self.dhcp_start, self.dhcp_end, self.dhcp_network)
        {
            let range = crate::common::config::dhcp_range::DhcpRange {
                network,
                start,
                end,
            };
            range.validate()?;
            cfg.set_dhcp_range(Some(range));
        }
